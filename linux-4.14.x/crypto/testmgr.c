	...
	for (i = 0; i < tcount; i++) {
		...
		if (template[i].ksize) {
			...
			if (ret) {
				...
				if (template[i].ksize > (512/8)) {
					printk(KERN_DEBUG "alg: hash: key too big for HW "
					       "test %d for %s: keylen=%d\n", j, algo,
					       template[i].ksize);
					ret = 0;
				} else
#endif
...
#ifdef	CONFIG_WG_ARCH_X86_64 // WG:JB Disable tests for QAT until fully implemented by Intel.
	if (strncmp(driver, "qat_", 4) == 0) goto notest;
#endif
...
test_done:
	...
	if (fips_enabled && !rc)
#ifdef	CONFIG_WG_PLATFORM // WG:DW Tone down non-essential INFO messages
