...
#ifdef CONFIG_WG_ARCH_X86 // WG:TW VMs want this
static u32 ne2k_pci_get_link(struct net_device *dev) {
    return 1;
}

static int ne2k_pci_get_settings(struct net_device *dev, struct ethtool_cmd *cmd) {

	//printk(KERN_ERR "ne2k_pci_get_settings called for %s\n",dev->name);
	cmd->supported      = SUPPORTED_100baseT_Half|SUPPORTED_100baseT_Full|SUPPORTED_1000baseT_Full|SUPPORTED_Autoneg;
	cmd->advertising    = ADVERTISED_100baseT_Full|ADVERTISED_1000baseT_Full|ADVERTISED_Autoneg;
	cmd->speed      = SPEED_100;
...
static int ne2k_pci_set_settings(struct net_device *dev, struct ethtool_cmd *cmd) {

    //    printk(KERN_ERR "ne2k_pci_set_settings called for %s\n",dev->name);
	return 0;
}
#endif	// CONFIG_WG_ARCH_X86 // WG:TW VMs want this
...
#ifdef CONFIG_WG_ARCH_X86 // WG:TW VMs want this
	.get_link		= ne2k_pci_get_link,
	.get_settings		= ne2k_pci_get_settings,
	.set_settings		= ne2k_pci_set_settings,
#endif	// CONFIG_WG_ARCH_X86 // WG:TW VMs want this
};
