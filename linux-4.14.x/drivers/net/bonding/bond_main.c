...
#ifdef	CONFIG_WG_PLATFORM // WG:JB Fix for BUG86879

#define	WG_NETIF_F_OPT		(NETIF_F_ALL_TSO)

static netdev_features_t wg_bond_get_opt(struct bonding *bond)
{
	struct slave *slave;
	struct list_head *iter;
	netdev_features_t opt = WG_NETIF_F_OPT;

	if (!bond_has_slaves(bond))
		return 0;

	bond_for_each_slave(bond, slave, iter) {
		opt &= (slave->dev->features & WG_NETIF_F_OPT);
	}
...
#ifdef	CONFIG_WG_PLATFORM // WG:JB Fix for BUG86879
	{
		netdev_features_t opt = wg_bond_get_opt(bond);
		bond_dev->vlan_features &= ~WG_NETIF_F_OPT;
		bond_dev->vlan_features |= opt;
		bond_dev->hw_features   &= ~WG_NETIF_F_OPT;
		bond_dev->hw_features   |= opt;
		bond_dev->features      &= ~WG_NETIF_F_OPT;
		bond_dev->features      |= opt;
	}
#endif
...
#ifdef CONFIG_WG_PLATFORM	//WG:XD fbx-22265
	local_bh_disable();
#endif
...
#ifdef	CONFIG_WG_PLATFORM // WG:JB Fix for BUG86879
	bond_dev->hw_features &= ~WG_NETIF_F_OPT;
	bond_dev->hw_features |= wg_bond_get_opt(bond);
#endif
