...
#ifdef	CONFIG_WG_PLATFORM // WG:JB SMI command register
#undef	SMI_CMD
#undef	SMI_DATA
#define	SMI_CMD		(wg_dsa_smi_reg|0)
#define	SMI_DATA	(wg_dsa_smi_reg|1)
...
#else	// CONFIG_WG_PLATFORM_PATCHES
	for (i = 0; i < 16; i++) {
		ret = mdiobus_read(bus, sw_addr, SMI_CMD);
		if (ret < 0)
			return ret;
...
#ifdef	CONFIG_WG_PLATFORM // WG:JB PHY mapping
			    SMI_CMD_OP_22_READ | (DSA_PHY_MAP(addr) << 5) | reg);
#else
...
static int _mv88e6xxx_reg_read(struct dsa_switch *ds, int addr, int reg)
{
	...
	struct mii_bus *bus = dsa_host_dev_to_mii_bus(NULL);
#else
...
#ifdef	CONFIG_WG_KERNEL_4_14
	ret = __mv88e6xxx_reg_read(bus, ds->cd->sw_addr, addr, reg);
	if (ret < 0)
		return ret;
#else
			  ...
			  int reg, u16 val)
{
	...
	ret = mdiobus_write(bus, sw_addr, SMI_CMD,
#ifdef	CONFIG_WG_PLATFORM // WG:JB PHY mapping
			    SMI_CMD_OP_22_WRITE | (DSA_PHY_MAP(addr) << 5) | reg);
#else
...
#ifdef	CONFIG_WG_KERNEL_4_14
	return __mv88e6xxx_reg_write(bus, ds->cd->sw_addr, addr, reg, val);
#else
	dev_dbg(ds->master_dev, "-> addr: 0x%.2x reg: 0x%.2x val: 0x%.4x\n",
		addr, reg, val);
...
void mv88e6xxx_poll_link(struct dsa_switch *ds)
{
...
#ifdef	CONFIG_WG_KERNEL_4_14
		int sw = ds->cd->sw_addr & 1;
		struct net_device* dev = ds->dst->cpu_dp->netdev;
#else
	...
	for (i = 0; i < DSA_MAX_PORTS; i++) {
		...
		case PORT_STATUS_SPEED_1000:
			speed = 1000;
			break;
		case PORT_STATUS_SPEED_2500:	/* WG: Talor Lin added for FBX-16785 */
			speed = 2500;
			break;
		default:
			speed = -1;
		...
		if (!netif_carrier_ok(dev)) {
			netdev_info(dev,
				    ...
				    duplex ? "full" : "half",
				    fc ? "en" : "dis");
#ifdef CONFIG_WG_PLATFORM // WG:XD FBX-17134
			if (likely(dev != ds->dst->cpu_dp->netdev))
				netif_carrier_on(dev);
			else
				printk(KERN_INFO "@@ %s: defer to the MAC driver for %s to set its link flag on/s\n",
...
#ifdef CONFIG_WG_PLATFORM	// WG:XD FBX-17094
	if (unlikely(in_irq())) {
...
#ifndef	CONFIG_WG_PLATFORM_PATCHES // WG:JB Removed unused code
