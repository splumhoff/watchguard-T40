...
#ifdef CONFIG_WG_PLATFORM	// WG:XD FBX-19824
...
#ifdef	CONFIG_WG_PLATFORM // WG:JB Check for root device
		{

			extern void wg_check_rootdev(char*, u32);
			// WG:XD: FBX-19824 if the root dev has already been successfully mounted, then we don't have to update the root dev on our UTM boxes again, whether the newly detected dev is USB or other type of devices.
			// WG:MJ: FBX-22128/FBX-20668: If the device under queston is a USB device, we should just ignore all of this and move on.
			// All USB devices have a vendor ID of !"ATA". All HDD devices have a vendor ID of "ATA". Great!
			if ((sys_access("/sbin/init", 0) != 0 && sys_access("/bin/init", 0) != 0) &&   // FBX-19824
			    (sdp->vendor && memcmp(sdp->vendor, "ATA", 3) == 0)) { // FBX-22128/FBX-20668
				sd_printk(KERN_NOTICE, sdkp, "non-USB storage device detected\n");
				wg_check_rootdev(sdkp->disk->disk_name,
					sdkp->capacity >> 11);
			} else {
				sd_printk(KERN_NOTICE, sdkp, "USB storage device detected, %s\n", sdp->vendor ? sdp->vendor: "void");
			}
		}
#endif
	}
