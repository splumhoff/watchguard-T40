...
#endif // defined(CONFIG_WG_PLATFORM_M590_M690)
	/* FQ stats */
	"rx pending frames",
	"rx pending bytes",
...
#if defined(CONFIG_WG_PLATFORM_M590_M690)
	if (strcmp(net_dev->name, "eth8") == 0 ||
		strcmp(net_dev->name, "eth9") == 0 ) {
		printk_ratelimited(KERN_INFO "@@ settings for %s SFP is fixed at 10G, won't change\n",
						   net_dev->name);
		return -EACCES;
	}
#endif
...
#if defined(CONFIG_WG_PLATFORM_M590_M690) 
	/* Note: Will need to confirm if the following code is still needed since "congestion state" related code has been removed by Liteon */
	if(priv->cscn_mem != NULL) {
		*(data + i++) = dpaa2_cscn_state_congested(priv->cscn_mem);
	} else {
		*(data + i++) = 0;
		printk("Func %s: line %d, seems like \"[drv] tx congestion state\" is not supported in ethtool statistics!\n", __func__, __LINE__);
	}
#endif // defined(CONFIG_WG_PLATFORM_M590_M690)
