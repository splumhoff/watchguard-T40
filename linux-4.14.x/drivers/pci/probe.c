...
#ifdef	CONFIG_WG_PLATFORM // WG:JB Only scan certain buses when in kdump
#ifdef	CONFIG_CRASH_DUMP
#ifdef	CONFIG_X86
	{
	  int wg_pci_bus_scan = 0; // Skip scan bit map, default is scan all
	  struct cpuinfo_x86* info = &cpu_data(0);
	  char* model = &info->x86_model_id[0];
	  // Only scan first bus on Finley E5410 CPU
	  if (strstr(model, "E5410")) wg_pci_bus_scan = ~1;
	  if ((1 << (bus->number)) & wg_pci_bus_scan) {
	    printk(KERN_EMERG "PCI: Skip kdump bus %02x\n", bus->number);
	    return 0;
	  }
	}
#endif
