...
#ifdef	CONFIG_WG_ARCH_X86 // WG:JB Do not create the Prestera resource here
	if (unlikely(wg_seattle))
	if ((dev->vendor == 0x8086) && (dev->device == 0x10FB)) {
		printk(KERN_WARNING "Prestera Resource %d:%02d delayed allocation.\n", dev->devfn,  resno);
		return 0;
	}
#endif
