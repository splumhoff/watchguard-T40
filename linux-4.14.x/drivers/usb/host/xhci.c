...
int xhci_reset(struct xhci_hcd *xhci)
{
...
#if defined(CONFIG_WG_PLATFORM_M390) || defined(CONFIG_WG_PLATFORM_M590_M690)
	/* Hack xhci SuperSpeed PORTSC field Link State for CP0 trigger */
	xhci_regs = ioremap(0x3100000, 0xffff);
	val = readl(xhci_regs + 0x430);
	DBG_PRINTF("Printf USB Before write val: 0x%0x\n", val);
	val = 0x010340;
	writel(val, xhci_regs + 0x430);
	mdelay(500);
#if defined(CONFIG_WG_PLATFORM_M390)
	val = readl(xhci_regs + 0x430);
#endif
	DBG_PRINTF("Printf USB read_val: 0x%0x\n", val);
	iounmap(xhci_regs);
	...
	xhci_regs = ioremap(0x3110000, 0xffff);
	val = readl(xhci_regs + 0x430);
	DBG_PRINTF("USB2 Before write val: 0x%0x\n", val);
	val = 0x010340;
	writel(val, xhci_regs + 0x430);
	mdelay(500);
#if defined(CONFIG_WG_PLATFORM_M390)
	val = readl(xhci_regs + 0x430);
#endif
	DBG_PRINTF("USB2 After read val: 0x%0x\n", val);
	iounmap(xhci_regs);

#endif // M390 || M590_M690
