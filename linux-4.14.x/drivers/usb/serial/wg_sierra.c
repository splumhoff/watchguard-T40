...
static DEVICE_ATTR(suspend_status, S_IRUSR | S_IRUGO, show_suspend_status,
							NULL);

/* Read only statistics */
static DEVICE_ATTR(stats, S_IRUSR | S_IRUGO, show_stats, NULL);
...
static void sierra_indat_callback(struct urb *urb)
{
	...
	if (status) {
		...
		if (urb->actual_length) {
			struct tty_port *t_port = &port->port;
			tty = tty_port_tty_get(&port->port);
			if (tty) {
				tty_insert_flip_string(t_port, data,
					urb->actual_length);
