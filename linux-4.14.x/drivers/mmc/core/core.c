...
void mmc_wait_for_req_done(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	while (1) {
#ifdef	CONFIG_WG_PLATFORM // WG:JB Don't wait forever!
		int timeout = wait_for_completion_timeout(&mrq->completion, 2*HZ);
		if (unlikely(timeout <= 0)) mrq->cmd->error = -ETIMEDOUT;
#else
