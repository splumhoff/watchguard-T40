...
#ifndef CONFIG_WG_PLATFORM_BACKPORT	// WG:XD FBX-18418
		if (likely(!ret))
			return 0;
#else
		if (likely(!ret)) {
...
#ifdef CONFIG_WG_PLATFORM_BACKPORT	// WG:XD FBX-18418
...
#ifdef CONFIG_WG_PLATFORM	// WG: XD FBX-2202
	local_bh_disable();
...
#ifdef	CONFIG_WG_ARCH_FREESCALE // WG:JB Suppress ICV errors
		if ((console_loglevel & 16) ||
		    ((ssrc != JRSTA_SSRC_CCB_ERROR) ||
