 ...
 * crypto alg
 */
#ifdef	CONFIG_WG_PLATFORM	// WG:XD FBX-17940, from 3.12 kernel
					...
					enum optype type)
{
	...
	if (unlikely(!drv_ctx)) {
#ifdef	CONFIG_WG_PLATFORM // WG:JB BUG 87053
		int tries = 10;
		while (spin_trylock_bh(&ctx->lock) == 0) {
			if (--tries >= 0) {
				udelay(2);
				continue;
			}
			printk(KERN_ERR "%s: ctx %p type %d locked\n",
			       __FUNCTION__, ctx, type);
					   ...
					   bool encrypt)
{
	...
	if (unlikely(wg_fips_iv)) {
	  memcpy(req->iv, wg_fips_iv, ivsize); wg_fips_iv = NULL;
	}
#endif
