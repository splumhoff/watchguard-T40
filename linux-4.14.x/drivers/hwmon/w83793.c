...
static int p2020;		/* flag if we have a P2020 */
#endif
...
#ifdef CONFIG_WG_PLATFORM  // WG:PE FBX-16730 ... from 3.12.19 (originally from lanner patch for 2.6.35)
    if(index == 3 )
        val = val * 16;
    else if(index == 6 )
        val = val * 11;
...
static void w83793_init_client(struct i2c_client *client)
{
	...
	w83793_write_value(client, W83793_REG_CONFIG,
			   w83793_read_value(client, W83793_REG_CONFIG) | 0x09);
#else
...
#ifdef	CONFIG_WG_ARCH_FREESCALE
        if (p2020) {
		/* WG:BH - Again, more changes from Lanner for chelan */
		w83793_write_value(client, 0xb0, 0x5);
		w83793_write_value(client, 0xb1, 0x5);
	}
#endif
...
{
	if (wg_get_cpu_model() == 2020) p2020 = 2020;
