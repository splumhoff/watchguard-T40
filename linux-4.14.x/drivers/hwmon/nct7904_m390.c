			 ...
			 const struct i2c_device_id *id)
{
	...
	/* Castle Rock */
	/* LTD */
	/* Enable EN_VSEN 17,19, Disabled EN_VSEN18, it will not control FAN3 PWM */
	nct7904_write_reg(data, BANK_0, VT_ADC_CTRL2_REG, 0x17);
	ret = nct7904_read_reg(data, BANK_0, VT_ADC_CTRL2_REG);
	if (ret < 0)
		return ret;
	if ((ret & 0x02) == 0x02)
	...
	for (i = 0; i < FANCTL_MAX; i++) {
		...
		if (ret < 0)
			return ret;
		data->fan_mode[i] = ret;
		//printk(KERN_DEBUG "init fan mode = %d@@pwm\n",ret);
	}
