...
#ifdef	CONFIG_WG_ARCH_X86 // WG:JB kdump on Atom fails if events are enabled
#ifdef	CONFIG_CRASH_DUMP
	if (wg_cpu_model <= 0)
		printk(KERN_EMERG "NO CPU MODEL (%d) !!!\n", wg_cpu_model);
	if (wg_seattle != 0) {
		int i;
		u64 val;

#ifdef	CONFIG_SMP
		setup_max_cpus = 1; // Only start 1 CPU as well
#endif
		for (i = 0; i < x86_pmu.num_counters; i++) {
			rdmsrl(x86_pmu_config_addr(i), val);
			val &= ~ARCH_PERFMON_EVENTSEL_ENABLE;
			wrmsrl(x86_pmu_config_addr(i), val);
		}
	}
#endif
#endif	// WG:JB kdump changes
...
#ifdef	CONFIG_WG_PLATFORM // WG:JB Turn off HW perf in kdump kernel
#ifdef	CONFIG_CRASH_DUMP
#ifdef	CONFIG_SMP
	setup_max_cpus = 1; // Only start 1 CPU as well
#endif
