...
static int dsa_ds_apply(struct dsa_switch_tree *dst, struct dsa_switch *ds)
{
	...
	if (ds->ops->set_addr) {
        ...
        if (dst->cpu_dp2) {
            err = ds->ops->set_addr(ds, dst->cpu_dp2->netdev->dev_addr);
            if (err < 0)
                return err;
        }
 #endif
...
static int dsa_dst_apply(struct dsa_switch_tree *dst)
{
 ...
 #if (!defined(CONFIG_WG_PLATFORM_M390) && !defined(CONFIG_WG_PLATFORM_M590_M690))
	if (dst->cpu_dp2) {
		err = dsa_cpu_port_ethtool_setup(dst->cpu_dp2);
		if (err)
			return err;
	}
 #endif
 ...
 #if (!defined(CONFIG_WG_PLATFORM_M390) && !defined(CONFIG_WG_PLATFORM_M590_M690))
    if (dst->cpu_dp2) {
        dst->cpu_dp2->netdev->dsa_ptr = dst;
    }
 #endif
#endif
	dst->applied = true;
...
static void dsa_dst_unapply(struct dsa_switch_tree *dst)
{
 ...
 #if (!defined(CONFIG_WG_PLATFORM_M390) && !defined(CONFIG_WG_PLATFORM_M590_M690))
    if(dst->cpu_dp2)
    	dst->cpu_dp2->netdev->dsa_ptr = NULL;
 #endif
#endif
 ...
 #if (!defined(CONFIG_WG_PLATFORM_M390) && !defined(CONFIG_WG_PLATFORM_M590_M690))
	if (dst->cpu_dp2) {
		dsa_cpu_port_ethtool_restore(dst->cpu_dp2);
		dst->cpu_dp2 = NULL;
	}
 #endif
#endif
...
#ifdef CONFIG_WG_PLATFORM_DSA_MODE_T80
 #if (!defined(CONFIG_WG_PLATFORM_M390) && !defined(CONFIG_WG_PLATFORM_M590_M690))
static int dsa_of_probe_user_port(struct dsa_switch *ds,
                  struct dsa_port *dp,
				  int port_index)
{
	struct device_node *cpu_port;
	const unsigned int *cpu_port_reg;
	int cpu_port_index = -1;
    struct device_node *port = dp->dn;

    if(!ds)
    {
        pr_err("DSA: port %d, NULL dsa switch\n", port_index);
        return -EINVAL;
    }
    if(!port)
    {
        pr_err("DSA: port %d, NULL port device node\n", port_index);
        return -EINVAL;
    }
	...
	if (cpu_port) {
		...
		ds->port_cpu[port_index] = cpu_port_index;
        dp->cpu_dp = &ds->ports[cpu_port_index];
	}
			 ...
			 struct dsa_switch *ds)
{
	...
	if (port->dn) {
#if defined(CONFIG_WG_PLATFORM_M390) || defined(CONFIG_WG_PLATFORM_M590_M690)
		ethernet_dev = __dev_get_by_name(&init_net, "eth0"); // MJ : ??? Ask Albert
		if (NULL == ethernet_dev)
		{
			dev_err(ds->dev, "eth0 dev is NULL!!!!\n");
			return -EPROBE_DEFER;
		}
		dev_warn(ds->dev, "dev->name[%s],mtd[%d]\n", ethernet_dev->name, ethernet_dev->mtu);
#else
	...
	if (!dst->cpu_dp) {
		...
		dst->cpu_dp2 = port;
		dst->cpu_dp2->netdev = ethernet_dev;
	}
	ds->port_ethernet[index] = ethernet_dev;
#endif
...
#ifdef CONFIG_WG_PLATFORM_DSA_MODE_T80
 #if (!defined(CONFIG_WG_PLATFORM_M390) && !defined(CONFIG_WG_PLATFORM_M590_M690))
			dp->cpu_dp2 = dst->cpu_dp2;
			if (!dsa_port_is_dsa(dp) &&
			    !dsa_port_is_cpu(dp))
			{
				err = dsa_of_probe_user_port(ds, dp, port);
				if (err)
				{
					pr_warn("Failed to probe user port %d\n", port);
				}
			}
 #endif
#endif /* CONFIG_WG_PLATFORM_DSA_MODE_T80 */
