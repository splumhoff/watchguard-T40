				      ...
				      struct jffs2_raw_node_ref *raw, struct jffs2_inode_info *f)
{
	...
	if (fd && fd->ino) {
		...
		if (ref_obsolete(raw)) {
			...
			/* supposted to be a temporary Fix specific to richland:
			   Now, as we are paranoid how bad this is going to when we release 11.5.2 for new* and chelan2,
			   we are making it a fix for every platform :( .
			 */
			panic("<%s>: Raw node: wasn't in node lists & wasn't obsolete: how can this happen?\n", __FUNCTION__);
#else
			BUG();
#endif
		}
