...
void
MJ_jffs2_dbg_dump_jeb_nolock(struct jffs2_sb_info *c, struct jffs2_eraseblock *jeb)
{
	if (!jeb)
		return;
	...
	printk("flash_size: %#08x\n",		c->flash_size);
	printk("used_size: %#08x\n",		jeb->used_size);
	printk("dirty_size: %#08x\n",		jeb->dirty_size);
	printk("wasted_size: %#08x\n",	jeb->wasted_size);
	printk("unchecked_size: %#08x\n",	jeb->unchecked_size);
	printk("free_size: %#08x\n",		jeb->free_size);
	printk("erasing_size: %#08x\n",	c->erasing_size);
	printk("bad_size: %#08x\n",		c->bad_size);
}
#endif
...
#ifdef	CONFIG_WG_PLATFORM
		// Just in case, if I need for the future
		// MJ_jffs2_dbg_dump_jeb_nolock(c, jeb);
#endif
	}
