...
static void dsa_link_poll_work(struct work_struct *ugly)
{
	...
	for (i = 0; i < dst->pd->nr_chips; i++) {
		...
		if (ds != NULL && ds->ops->poll_link != NULL)
			ds->ops->poll_link(ds);
	}
				...
				struct net_device *master)
{
	...
	for (i = 0; i < ds->num_ports; i++) {
		...
		if (!strcmp(name, "cpu")) {
			...
			if(!dst->cpu_dp2)
			{
				dst->cpu_dp2 = &ds->ports[i];
				dst->cpu_dp2->netdev = master;
			}
			ds->cpu_port_mask |= 1 << i;
		} else if (!strcmp(name, "dsa")) {
#endif
	...
	if (dst->cpu_dp->ds == ds) {
		...
		if (!ops || !ops->get_tag_protocol) {
			printk(KERN_WARNING "@@ %s ops is NULL!\n", __FUNCTION__);
			return -1;
		}
	...
	for (i = 0; i < ds->num_ports; i++) {
		...
		ds->ports[i].cpu_dp2 = dst->cpu_dp2;
 #endif
#endif
    ...
    if(ds->dst->cpu_dp2)
    {
    	ret = dsa_cpu_port_ethtool_setup(ds->dst->cpu_dp2);
    	if (ret)
    		return ret;
    }
 #endif
...
struct mii_bus *dsa_host_dev_to_mii_bus(struct device *dev)
{
	struct device *d;
#ifndef CONFIG_WG_PLATFORM_DSA_MODE_T80
#ifdef	CONFIG_WG_PLATFORM // WG:JB Return mii bus if we know it
	if (likely(wg_dsa_bus)) return wg_dsa_bus;
	if (!dev) {
		printk(KERN_EMERG "%s: No device\n", __FUNCTION__);
		return NULL;
	}
...
#ifndef CONFIG_WG_PLATFORM_DSA_MODE_T80
#ifdef	CONFIG_WG_PLATFORM // WG:JB Save away mii bus
		wg_dsa_bus = bus;
#endif	// CONFIG_WG_PLATFORM
#endif // CONFIG_WG_PLATFORM_DSA_MODE_T80
		return bus;
...
#ifndef CONFIG_WG_PLATFORM_DSA_MODE_T80
#ifdef CONFIG_WG_PLATFORM // WG:JB We pre-build all this in wg_dsa.c
#undef CONFIG_OF
#endif // CONFIG_WG_PLATFORM
#endif // CONFIG_WG_PLATFORM_DSA_MODE_T80
#ifdef CONFIG_OF
