...
static int register_algorithms = 1; /* default is '1'. It registers all algorithms */
module_param(register_algorithms, int, 0);
MODULE_PARM_DESC(register_algorithms,
		 "If set to zero, the driver will not register any CAAM HW algorithm at all.");
...
{
	if (register_algorithms == 0) {
		dev_info(dev, "User req: Switch to SW-Crypto, no HW alg registered\n");
		return;
	}
...
{
	if (register_algorithms == 0) {
		printk("User req'd SW-Crypto at load, nothing to unregister\n");
		return;
	}
...
static int caam_jr_probe(struct platform_device *pdev)
{
	...
	printk("save ring identity relative to detection\n");
