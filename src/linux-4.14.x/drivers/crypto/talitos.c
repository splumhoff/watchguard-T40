		   ...
		   void *context)
{
	...
	if (!atomic_inc_not_zero(&priv->chan[ch].submit_count)) {
		...
		printk(KERN_DEBUG "%s: fifo full ch%d enq%12u deq%12u #%4d\n",
		       __FUNCTION__, (int)ch,
		       atomic_read(&priv->chan[ch].enq_count),
		       atomic_read(&priv->chan[ch].deq_count),
		       -atomic_read(&priv->chan[ch].submit_count));
		priv->chan[ch].high_water = 0;
#endif
		return -EBUSY;
#else
		return -EAGAIN;
...
#ifdef	CONFIG_WG_GIT
		printk(KERN_EMERG "%s: ch %d vhi %08x vlo %08x hdr %08x\n",
		       __FUNCTION__, ch,
		       in_be32(priv->chan[ch].reg + TALITOS_CCPSR_LO),
		       v_lo, current_desc_hdr(dev, ch));
#endif
...
#ifdef	CONFIG_WG_GIT
	u8 key[(256/8)+(512/8)];	// AES256+SHA512
	unsigned int enclenmask;
#else
...
#ifdef	CONFIG_WG_GIT
	if (unlikely(console_loglevel & 16))
	printk(KERN_DEBUG "%s: enckeylen %d authkeylen %d\n",
	       __FUNCTION__,  keys.enckeylen,  keys.authkeylen);

	if (keys.enckeylen  > (256/8)) // AES256
		goto badkey;
	if (keys.authkeylen > (512/8)) // SHA512
		goto badkey;
	{
		unsigned int ivsize = crypto_aead_ivsize(authenc);
		ctx->enclenmask = ivsize ? (ivsize - 1) : 0;
	}
#else
...
#ifdef	CONFIG_WG_GIT // WG:JB Only check ICV if authsize > 0
	if (likely(authsize > 0))
#endif
	if (!err) {
...
#ifdef	CONFIG_WG_GIT // WG:JB Only check ICV if HMAC set
	if (likely(!err)) {
	   if     (likely(edesc->desc.hdr & DESC_HDR_MODE1_MDEU_HMAC)) {
	     if (unlikely((desc->hdr_lo   & DESC_HDR_LO_ICCR1_MASK) !=
					    DESC_HDR_LO_ICCR1_PASS))
		err = -EBADMSG;
	   } else
	   if   (unlikely(edesc->desc.hdr & DESC_HDR_MODE0_MDEU_HMAC)) {
	     if (unlikely((desc->hdr_lo   & DESC_HDR_LO_ICCR0_MASK) !=
						 ...
						 bool encrypt)
{
	...
	dma_addr_t iv_dma = 0;
#ifdef	CONFIG_WG_GIT // WG:JB If in atomic, allocate atomically
	...
	if (cryptoflags & CRYPTO_TFM_REQ_MAY_SLEEP)
		flags = GFP_KERNEL;
#endif
...
#ifdef	CONFIG_WG_GIT // WG:JB Validate crypto length
	if (unlikely(areq->cryptlen & ctx->enclenmask)) {
		printk(KERN_EMERG "%s: cryptlen %d mod ivsize %d != 0\n",
		       __FUNCTION__, areq->cryptlen, ctx->enclenmask + 1);
		return ERR_PTR(-EINVAL);
	}
#endif
...
#ifdef	CONFIG_WG_GIT // WG:JB Only set CICV if HMAC set
		edesc->desc.hdr = ctx->desc_hdr_template |
				  DESC_HDR_DIR_INBOUND;

		if (  likely(edesc->desc.hdr &  DESC_HDR_MODE1_MDEU_HMAC))
			     edesc->desc.hdr |= DESC_HDR_MODE1_MDEU_CICV;
		else
		if (unlikely(edesc->desc.hdr &  DESC_HDR_MODE0_MDEU_HMAC))
			     edesc->desc.hdr |= DESC_HDR_MODE0_MDEU_CICV;
#else
		edesc->desc.hdr = ctx->desc_hdr_template |
				  DESC_HDR_DIR_INBOUND |
...
#ifdef	CONFIG_WG_PLATFORM_FIPS // WG:JB Allow FIPS to specify the IV
	if (unlikely(wg_fips_iv)) {
	  memcpy(req->iv, wg_fips_iv, crypto_aead_ivsize(authenc));
	  wg_fips_iv = NULL;
	}
#endif
...
#ifdef	CONFIG_WG_PLATFORM_FIPS // WG:JB Allow FIPS to specify the IV
	if (unlikely(wg_fips_iv)) {
	  areq->src->offset += ivsize;
	  areq->src->length -= ivsize;
	  wg_fips_iv = NULL;
	}
#endif
	sg_count = edesc->src_nents ?: 1;
...
#ifdef	CONFIG_WG_GIT // WG:JB Validate crypto length
	if (unlikely(areq->nbytes & ctx->enclenmask)) {
		printk(KERN_EMERG "%s: nbytes %d mod ivsize %d != 0\n",
		       __FUNCTION__, areq->nbytes, ctx->enclenmask + 1);
		return ERR_PTR(-EINVAL);
	}
#endif
	return talitos_edesc_alloc(ctx->dev, areq->src, areq->dst,
...
#ifdef	CONFIG_WG_PLATFORM_FIPS // WG:JB Allow FIPS to specify the IV
	if (unlikely(wg_fips_iv))
	  memcpy(areq->info, wg_fips_iv, crypto_ablkcipher_ivsize(cipher));
#endif
...
#ifndef CONFIG_WG_PLATFORM // WG:XD FBX-17625
...
#ifdef	CONFIG_WG_PLATFORM
		if (!strncmp(alg->cra_name, "auth", 4))
			t_alg->algt.alg.hash.setkey = ahash_setkey;
#endif
