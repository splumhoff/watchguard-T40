...
#include <linux/of_address.h>
#include <linux/of_irq.h>
#ifdef  CONFIG_WG_PLATFORM // WG:JB BUG83133
...
static void wg_gfar_dump(struct net_device *dev)
{
...
#if 0 // 2nd set of regs may not exist on some platforms
	regs = priv->gfargrp[1].regs;
	GFAR_DUMP_REG("ievent[1]", ievent);
	GFAR_DUMP_REG("imask[1]", imask);
	GFAR_DUMP_REG("tstat[1]", tstat);
#endif
	...
	printk(KERN_WARNING "tx_queue_stopped_count: %lu\n", dev->stats.tx_fifo_errors);
...
#define NUM_TO_DUMP		4
	for (i = 0; i < priv->num_tx_queues; i++) {
		...
		printk(KERN_WARNING "@@ %s # of Qs: %d, Q[%02d], ntc: %d, ntu: %d, tdh: %d, "
			   ...
			   atomic_read(&tx_queue->skb_curtx) & TX_RING_MOD_MASK(tx_queue->tx_ring_size),
...
static void gfar_init_bds(struct net_device *ndev)
{
	...
	for (i = 0; i < priv->num_tx_queues; i++) {
		...
		tx_queue->dirty_tx = tx_queue->tx_bd_base;
		tx_queue->cur_tx = tx_queue->tx_bd_base;
#ifdef	CONFIG_WG_PLATFORM_GFAR_TX_ATOMIC // WG:JB Make atomic
		atomic_set(&tx_queue->skb_curtx, -1);
#else
...
#ifdef	CONFIG_WG_PLATFORM_GFAR_RX_ATOMIC // WG:JB Make atomic
		atomic_set(&rx_queue->next_to_clean, -1);
		atomic_set(&rx_queue->next_to_use,   -1);
		atomic_set(&rx_queue->next_to_alloc, -1);
#else
...
#if defined(CONFIG_WG_PLATFORM_DSA_MODULE) || defined (CONFIG_WG_PLATFORM_DSA) // WG:JB Fix for L2OFF
	// If doing RX parsing, we need to set the parser's L2 offset
	if (rctrl & RCTRL_PRSDEP_INIT) {

		int l2_off = priv->wg_L2_off;

		// L2 offset must be an even number
		if ((l2_off &= -2) > 0) {

			// Set the L2 offset in RCTRL
			rctrl |= (l2_off << 24);
		...
		printk(KERN_INFO "%s: %s: L2OFF %2d  Pad %3d RCTRL %9x\n",
		       __FUNCTION__, priv->ndev->name,
		       l2_off, priv->padding, rctrl);
	}
...
#ifdef  CONFIG_WG_PLATFORM      // WG:JB BUG82891
...
#if defined(CONFIG_WG_PLATFORM_DSA_MODULE) || defined (CONFIG_WG_PLATFORM_DSA) // WG:JB Code to set L2OFF and eth table

static	inline void wg_fsl_rx_vlan_offload(struct sk_buff* skb, struct net_device* dev)
{
	struct ethhdr	*eth = (struct ethhdr *)skb->data;
	struct vlan_hdr	*vlan_header;
	u16		vlan_tag;

	if (unlikely(ETH_P_8021Q == eth->h_proto))
	if (likely(dev->features & NETIF_F_HW_VLAN_CTAG_RX)) {
		vlan_header = (struct vlan_hdr *)(skb->data + ETH_HLEN);
		vlan_tag = ntohs(vlan_header->h_vlan_TCI);
		__vlan_hwaccel_put_tag(skb, ETH_P_8021Q, vlan_tag);
				    ...
				    struct gfar_private *priv)
{
	...
	wg_fsl_rx_vlan_offload(skb, dev);

	/* Return what kind of packet this is */
	return eth_type_trans(skb, dev);
}

void wg_gfar_set_L2_off(struct net_device* sw_dev, int l2_off)
{
	struct gfar_private *priv = netdev_priv(sw_dev);

	if (priv) {
		priv->wg_L2_off = l2_off & -2;
	}
}
EXPORT_SYMBOL(wg_gfar_set_L2_off);
...
static int gfar_probe(struct platform_device *ofdev)
{
	 ...
	 */
	if (priv->device_flags & FSL_GIANFAR_DEV_HAS_TIMER)
#ifdef CONFIG_WG_GIT // WG:JB Add to padding rather than setting it to 8
		priv->padding |= 0x8;
#else
...
static void wg_no_advertise_eee(struct phy_device *phydev)
{
	...
	if (val) {
		printk(KERN_WARNING "PHY%02d: EEE advertise val = 0x%04x, set it to 0\n",
			   phydev->wg_mdio_addr, (unsigned int) val);
...
#ifndef CONFIG_WG_PLATFORM	// WG:XD FBX-16772
...
#ifdef	CONFIG_WG_PLATFORM_GFAR_TX_ATOMIC // WG:JB Make atomic
	tx_queue->tx_skbuff[atomic_inc_return(&tx_queue->skb_curtx) &
		TX_RING_MOD_MASK(tx_queue->tx_ring_size)] = skb;
#else
...
#ifdef CONFIG_WG_PLATFORM	// WG:XD FBX-16772
	// bottom-half need to be disabled here, already done by the fix for FBX-12174
...
#ifdef	CONFIG_WG_PLATFORM
	if (unlikely((skb->tail + size) > skb->end)) {
		printk(KERN_EMERG "%s: users %d head %p data %p tail %p end %p len %d %x\n",
		       __FUNCTION__, refcount_read(&skb->users), skb->head, skb->data,
		       skb->tail, skb->end, size, size);
		size = skb->end - skb->tail;
	}
#endif
