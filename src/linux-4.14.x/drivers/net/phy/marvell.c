...
static int marvell_config_aneg(struct phy_device *phydev)
{
	...
	if (phydev->autoneg != AUTONEG_ENABLE) {
		 ...
		 */
#ifdef CONFIG_WG_PLATFORM // WG:XD FBX-17217, code from 3.12.19 kernel
		int bmcr;
...
static int mvswitch_config_init(struct phy_device *phydev)
{
	printk(KERN_DEBUG "%s: %-16s PHY ADDR %2d ID %8x\n", __FUNCTION__,
	       phydev->drv->name, -1, phydev->phy_id);
...
#if defined(CONFIG_HWMON) && !defined(CONFIG_WG_PLATFORM_M590_M690)
