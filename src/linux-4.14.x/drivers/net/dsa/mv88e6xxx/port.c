...
#if defined(CONFIG_WG_PLATFORM_M290_T80) || defined(CONFIG_WG_PLATFORM_M390) || defined(CONFIG_WG_PLATFORM_M590_M690)
int mv88e6xxx_port_get_speed(struct mv88e6xxx_chip *chip, int port)
{
    int err, speed;
	u16 reg;
	...
	case MV88E6XXX_PORT_STS_SPEED_10: //10M or Link Down
		speed=SPEED_10;
		break;
...
#ifdef CONFIG_WG_PLATFORM_DSA_MODE_T80
int mv88e6xxx_port_get_vlan_map(struct mv88e6xxx_chip *chip, int port, u16 *map)
{
	...
	*map = reg & mask;
	...
	dev_dbg(chip->dev, "p%d: VLANTable get to 0x%.3x\n", port, *map);
...
int mv88e6xxx_port_set_vlan_map(struct mv88e6xxx_chip *chip, int port, u16 map)
{
	...
	const u16 mask = mv88e6xxx_port_mask(chip);
#else
	const u16 mask = mv88e6xxx_port_mask(chip) | MV88E6XXX_PORT_BASE_VLAN_FORCE_MAP;
#endif
	u16 reg;
	int err;
	...
	dev_dbg(chip->dev, "p%d: VLANTable set to 0x%.3x\n", port, map);
...
#ifdef CONFIG_WG_PLATFORM_DSA_MODE_T80
/* Offset 0x16: Port LED Control */

int mv88e6xxx_port_set_led_value(struct mv88e6xxx_chip *chip, int port,
				  u16 value)
{
	return mv88e6xxx_port_write(chip, port, MV88E6XXX_PORT_LED_CTL, value);
}

int mv88e6xxx_port_get_led_value(struct mv88e6xxx_chip *chip, int port,
				  u16 *value)
{
	return mv88e6xxx_port_read(chip, port, MV88E6XXX_PORT_LED_CTL, value);
}
#endif /* ifdef CONFIG_WG_PLATFORM_DSA_MODE_T80 */
