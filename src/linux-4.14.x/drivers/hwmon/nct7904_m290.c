...
#define GPIO_G1_EN          0xEC    /* Bank 0; GPIO Group 1 I/O Enable Control Register */
			    ...
			    long *val)
{
	...
	case hwmon_pwm_enable:
		...
		//printk(KERN_DEBUG "*val = %ld ret = %d@@pwm\n",*val,ret);
		*val = ret ? 2 : 1; //if echo value == previous value , echo value = 2, else return 1
		return 0;
	default:
		return -EOPNOTSUPP;
	}
			     ...
			     long val)
{
	struct nct7904_data *data = dev_get_drvdata(dev);
	int ret;
	int smart_mode[4] = {1,2,4,8}; //Temperature to Fan Mapping Relationships ,smart =2**(fan_num)

	switch (attr) {
	case hwmon_pwm_input:
		ret = nct7904_read_reg(data, BANK_3, FANCTL1_FMR_REG + channel); //read current fan_mode 
		if (ret < 0)
			return ret;

		if (val < 0 || val > 255 || ret != 0) //if current fan_mode is smartfan, can't set fan pwm
			return -EINVAL;
...
#else
		if (val < 1 || val > 2)  //can set smart,while manual mode.
		{
			return -EINVAL;
		}
		if(val == 2)//set smart fan
			val = smart_mode[channel];//Temperature to Fan Mapping Relationships ,smart =2**(fan_num)
		else
			val = 0;//manual mode = 0

		//printk(KERN_DEBUG "channel  = %d @@pwm\n",channel);
		ret = nct7904_write_reg(data, BANK_3, FANCTL1_FMR_REG + channel,val);
		//printk(KERN_DEBUG "write num = %d @@pwm\n",val);
#endif
		return ret;
	default:
		return -EOPNOTSUPP;
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (is_m290()) {
		ret |= 0x1c; /* 0x1c: Enable EN_VSEN17, EN_VSEN18, and EN_VSEN19 */
		nct7904_write_reg(data, BANK_0, VT_ADC_CTRL2_REG, ret);
		ret = nct7904_read_reg(data, BANK_0, VT_ADC_CTRL2_REG);
		printk("\nFunc %s: line %d, VT_ADC_CTRL2_REG (Address: 0x%x) = 0x%x\n", __func__, __LINE__, VT_ADC_CTRL2_REG, ret);
	}
	...
	/* Castle Rock */
	/* LTD */
	/* Enable enable EN_VSEN 17-19 */
	nct7904_write_reg(data, BANK_0, VT_ADC_CTRL2_REG, 0x1f);
	ret = nct7904_read_reg(data, BANK_0, VT_ADC_CTRL2_REG);
	if (ret < 0)
		return ret;
	if ((ret & 0x02) == 0x02)
	...
	nct7904_write_reg(data, BANK_0,VT_ADC_MD_REG, 0x5);
	ret = nct7904_read_reg(data, BANK_0, VT_ADC_MD_REG);
	if (ret < 0)
		return ret;

	/* Enable GPIO 13-15 */
	nct7904_write_reg(data, BANK_0, GPIO_G1_EN, 0x7);
	ret = nct7904_read_reg(data, BANK_0, GPIO_G1_EN);
	if (ret < 0)
		return ret;
	/* Castle Rock */
	...
	/*    Issue: [FBX-18601] T80 fan didn't turn on after system reboot under high temp
	   Solution: Add Liteon's solution to resolve the issue that smart fan mode can't be set successfully for some uncertain cases.
	*/
	ret = nct7904_write_reg(data, BANK_3, FANCTL1_FMR_REG, 1);
	if (ret < 0)
	{
		printk("Failed to set FAN %d to be SmartFAN mode\n", 1);
		return ret;
	}
	/* End of [FBX-18601] fix */
