...
#ifdef	CONFIG_WG_PLATFORM_FIPS // WG:JB Allow FIPS to specify IV
	if (unlikely(wg_fips_iv)) {
		memcpy(walk.src.virt.addr, wg_fips_iv, AES_BLOCK_SIZE);
		memcpy(walk.dst.virt.addr, wg_fips_iv, AES_BLOCK_SIZE);
		memcpy(walk.iv,		   wg_fips_iv, AES_BLOCK_SIZE);
		walk.src.virt.addr += AES_BLOCK_SIZE;
		walk.dst.virt.addr += AES_BLOCK_SIZE;
		walk.nbytes	   -= AES_BLOCK_SIZE;
		walk.total	   -= AES_BLOCK_SIZE;
		wg_fips_iv = NULL;
	}
#endif	// CONFIG_WG_PLATFORM_FIPS
...
static int rfc4106_wrapper_encrypt(struct aead_request *req)
{
	// WG:JB Adjust if called from ESP
	if (req->assoclen <  16)   {
	    req->assoclen +=  8;
	    req->cryptlen -=  8;
	    memcpy(sg_virt(req->src) + 8, req->iv, 8);
	}

	return gcmaes_wrapper_encrypt(req);
}

static int rfc4106_wrapper_decrypt(struct aead_request *req)
{
	// WG:JB Adjust if called from ESP
	if (req->assoclen <  16)   {
	    req->assoclen +=  8;
	    req->cryptlen -=  8;
	    memcpy(req->iv, sg_virt(req->src) + 8, 8);
	}

	return gcmaes_wrapper_decrypt(req);
}
...
#ifdef	CONFIG_WG_KERNEL_4_14 // WG:JB Adjust if called from ESP
	.encrypt		= rfc4106_wrapper_encrypt,
	.decrypt		= rfc4106_wrapper_decrypt,
#else
