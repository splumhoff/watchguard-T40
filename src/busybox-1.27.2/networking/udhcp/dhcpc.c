...
/* WatchGuard: port WG 1.18.2 changes: -c, -l options */
/* WatchGuard: Add -X option for RFC 3118 and -N option for RFC 3203. */
#if ENABLE_LONG_OPTS
static const char udhcpc_longopts[] ALIGN1 =
	...
	"leasetime\0"      Required_argument "l"
	"auth-token\0"     Required_argument "X"
	"force-renew\0"    No_argument       "N"
	"now\0"            No_argument       "n"
...
static void add_client_options(struct dhcp_packet *packet)
{
	...
	/* WatchGuard: port WG 1.18.2 changes:
	 * SCH: remove this, causes problems with comcast
...
static NOINLINE int send_discover(uint32_t xid, uint32_t requested)
{
	...
	if (requested)
 	...
 	/* WatchGuard: port WG 1.18.2 changes: * add -l leasetime option */
	if (client_config.lease_time)
		udhcp_add_simple_option(&packet, DHCP_LEASE_TIME, htonl(client_config.lease_time));
 ...
 */
	...
	udhcp_add_simple_option(&packet, DHCP_SERVER_ID, server);
 	...
 	/* WatchGuard: port WG 1.18.2 changes: Add -l lease_time option */
	if (client_config.lease_time)
		udhcp_add_simple_option(&packet, DHCP_LEASE_TIME, htonl(client_config.lease_time));
 ...
 */
	...
	/* WatchGuard: in WG1.12.1, we added the support for lease time in
	 * send_discover and send_selecting. The -l leasetime option should 
	 * also be sent in send_renew. */
	if (client_config.lease_time)
		udhcp_add_simple_option(&packet, DHCP_LEASE_TIME, htonl(client_config.lease_time));
...
//usage:       "	[-i IFACE]"IF_FEATURE_UDHCP_PORT(" [-P PORT]")" [-s PROG] [-l LEASETIME] [-p PIDFILE]\n"
//usage:       "	[-oC] [-r IP] [-H HOSTNAME] [-V VENDOR] [-F NAME] [-x OPT:VAL]... [-O OPT]..." IF_FEATURE_UDHCP_PORT(" [-P N]")
//usage:#define udhcpc_full_usage "\n"
//usage:	IF_LONG_OPTS(
//usage:     "\n	-c,--client CLIENTID	Client identifier (default MAC)"
//usage:     "\n	-i,--interface IFACE	Interface to use (default eth0)"
//usage:     "\n	-l,--leasetime=LEASETIME	Lease time to request"
...
//usage:	)
//usage:     "\n	-N,--force-renew	Allow DHCP FORCERENEW (RFC 3203)"
...
//usage:     "\n	-c CLIENTID	Client identifier (default MAC)"
//usage:     "\n	-i IFACE	Interface to use (default eth0)"
//usage:     "\n	-l LEASETIME	Lease time to request"
//usage:	IF_FEATURE_UDHCP_PORT(
...
int udhcpc_main(int argc UNUSED_PARAM, char **argv)
{
	...
	/* WatchGuard: port WG 1.18.2 changes: -c, -l options */
	const char *str_C, *str_V, *str_h, *str_F, *str_r, *str_l;
	...
	int retval;
	/* WatchGuard: Add support for DHCP message authentication (RFC 3118) */
	...
	/* WatchGuard: port WG 1.18.2 changes: -c, -l option */
	/* WatchGuard: Add -X option for RFC 3118 and -N option for RFC 3203. */
	client_config.lease_time = 0;
	/* WatchGuard: Add -X option for RFC 3118. */
	client_config.auth_token = NULL;
	str_V = "udhcp "BB_VER;
	...
	opt = getopt32(argv, "c:CV:H:h:F:i:l:X:Nnp:qRr:s:T:+t:+SA:+O:*ox:*fB"
		USE_FOR_MMU("b")
		IF_FEATURE_UDHCPC_ARPING("a::")
		IF_FEATURE_UDHCP_PORT("P:")
		"v"
		, &str_C, &str_V, &str_h, &str_h, &str_F
		, &client_config.interface, &str_l, &client_config.auth_token /* i, l, X */
		, &client_config.pidfile, &str_r, &client_config.script /* p, r, s */
	...
	/* WatchGuard: port WG 1.18.2 changes: -c option */
	if (opt & OPT_c)
	...
	if (opt & OPT_F) {
 	...
 	/* WatchGuard: WG 1.18.2 changes: -l leasetime option */
	if (opt & OPT_l)
		client_config.lease_time = xatoi_positive(str_l);
	/* WatchGuard: Add -N option for RFC 3203. */
	client_config.force_renew = (opt & OPT_N) ? 1 : 0;
	if (opt & OPT_r)
	...
	if (!(opt & OPT_C) && !(opt & OPT_c) && !udhcp_find_option(client_config.options, DHCP_CLIENT_ID)) {
	...
	state = INIT_SELECTING;
	/* WatchGuard -- port WG 1.18.2 changes: 
	  * don't default config at this point */
	/* udhcp_run_script(NULL, "deconfig"); */
	change_listen_mode(LISTEN_RAW);
	packet_num = 0;
	timeout = 0;
...
//FIXME: need to also check that last 10 bytes are zero
		...
		if ((DHCPFORCERENEW != *message)
			|| (0 == client_config.force_renew)) {
			if (packet.xid != xid) {
 ...
 * the configuration process..." */
				...
				state = BOUND;
				/* WatchGuard: If FORCERENEW is enable, keep listening. */
				if (0 == client_config.force_renew)
					change_listen_mode(LISTEN_NONE);
 ...
 non_matching_svid:
			...
			continue;
		case BOUND:
			/* WatchGuard: Add support for DHCP FORCERENEW (RFC 3203) and
			 * authentication (RFC 3118) */
			if ((*message == DHCPFORCERENEW)
				&& (client_config.force_renew != 0)) {
				bb_error_msg("Received DHCP FORCERENEW");
				if (client_config.auth_token) {
					struct dhcp_auth dhauth;
					/* Check server's authentication. */
					temp = udhcp_get_option(&packet, DHCP_AUTH);
					if (!temp) {
						bb_error_msg("DHCP FORCENEW has no authentication option");
						continue;
					}
					/*
					 * udhcp_get_option() gives a pointer to option data and
					 * verfies that the indicated length does not exceed the
					 * available data. However, for secure parsing, we need to
					 * know what that length is so we go back a byte to find it.
					 */
					dhauth.opt_len = *(temp-1);
					if (dhauth.opt_len <= DHCPAUTH_MIN_LEN) {
						bb_error_msg("DHCP FORCENEW authentication option is too small");
						continue;
					}
					dhauth.proto = *temp;
					dhauth.alg = *(temp+1);
					dhauth.rdm = *(temp+2);
					dhauth.auth_len = dhauth.opt_len - DHCPAUTH_MIN_LEN;
					if ((dhauth.proto != DHCPAUTH_PROTO_TOKEN)
						|| (dhauth.alg != DHCPAUTH_ALG_TEXT)
						|| (dhauth.rdm != DHCPAUTH_RDM_MONOCOUNTER)) {
						bb_error_msg("DHCP FORCENEW authentication type is not supported");
						continue;
					}
					/* For replay detection, ensure the monotonic counter is going up. */
					memcpy(&dhauth.replay_data, temp+3, sizeof(dhauth.replay_data));
					dhauth.replay_data = be64toh(dhauth.replay_data);
					if (dhauth.replay_data <= replay_data) {
						bb_error_msg("DHCP FORCENEW authentication replay detected");
						continue;
					}
					if ((strlen(client_config.auth_token) != dhauth.auth_len)
						|| (memcmp(temp+11, client_config.auth_token, dhauth.auth_len) != 0) ) {
						bb_error_msg("DHCP FORCENEW authentication failed");
						continue;
					}
					/* Only save the monotonic counter if both it and the
					 * authentication token are valid. */
					replay_data = dhauth.replay_data;
				}
				/* Stop listening for FORCERENEW */
				change_listen_mode(LISTEN_NONE);
				/* Instigate renewal by ajusting timeout and seconds waited. */
				timeout = 61;
				already_waited_sec = 60;
				packet_num = 0;
			}
			continue;
