...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ TODO: DHCPv6 has 16-bit option numbers
	...
	uint32_t lease_time;            /* Optional lease time to use */
	char force_renew;               /* Optional support for force renew (RFC 3203) */
	const char *auth_token;         /* Optional authentication token (RFC 3118) */
	uint16_t first_secs;
	uint16_t last_secs;
} FIX_ALIASING;
