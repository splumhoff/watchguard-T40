...
#else
	WR_PHY(name, (ports_ofs + 0),  MV88E61XX_PRT_VMAP_REG, 0x40);
	WR_PHY(name, (ports_ofs + 1),  MV88E61XX_PRT_VMAP_REG, 0x40);
	WR_PHY(name, (ports_ofs + 2),  MV88E61XX_PRT_VMAP_REG, 0x40);
	WR_PHY(name, (ports_ofs + 3),  MV88E61XX_PRT_VMAP_REG, 0x40);
	WR_PHY(name, (ports_ofs + 4),  MV88E61XX_PRT_VMAP_REG, 0x40);
	WR_PHY(name, (ports_ofs + 5),  MV88E61XX_PRT_VMAP_REG, 0x1f);
	WR_PHY(name, (ports_ofs + 6),  MV88E61XX_PRT_VMAP_REG, 0x1f);
#endif
...
int mv88e61xx_switch_initialize(struct mv88e61xx_config *swconfig)
{
	...
	if (swconfig->rgmii_delay == MV88E61XX_RGMII_DELAY_EN) {
		 ...
		 */
                RD_PHY(name, MV88E61XX_PRT_OFST + SC_eTSEC1_PORT,
                ...
                WR_PHY(name, MV88E61XX_PRT_OFST + SC_eTSEC1_PORT,
                                MV88E61XX_PORT_PHYCTRL_REG, sc_reg);

		RD_PHY(name, MV88E61XX_PRT_OFST + SC_eTSEC3_PORT, 
				MV88E61XX_PORT_PHYCTRL_REG, &sc_reg);		
				
		sc_reg |= (1 << MV88E61XX_PORT_PHYCTRL_REG_RXDELAY_BIT);
		sc_reg |= (1 << MV88E61XX_PORT_PHYCTRL_REG_TXDELAY_BIT);	

		WR_PHY(name, MV88E61XX_PRT_OFST + SC_eTSEC3_PORT, 
				MV88E61XX_PORT_PHYCTRL_REG, sc_reg);
	}
#else
	if (swconfig->rgmii_delay == MV88E61XX_RGMII_DELAY_EN) {
	...
	mv88e61xx_wr_phy(name, 0x1B, 0x4, 0x4000);
#endif
	printf("%s Initialized on %s\n", idstr, name);
	return 0;
}

#ifdef CONFIG_WGXTM_BOREN_NO
...
int sc_phy_read(int phy_id,int page,int reg)
{
    ...
    {
        printf("addr=0x%02x page=0x%02x reg=0x%02x data=0x",(uint)phy_id,(uint)page, (uint)reg);
        printf("%04X\n", data & 0x0000FFFF);
    }
...
#ifdef CONFIG_BOREN_ETHNET_SETUP_NO
