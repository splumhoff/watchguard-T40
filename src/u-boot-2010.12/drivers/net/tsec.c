...
static int tsec_initialize(bd_t * bis, struct tsec_info_struct *tsec_info)
{
	...
	eth_register(dev);
#ifndef CONFIG_WGXTM_BOREN /* AP331IB eTSEC1's CLK source */
    {
        u32 gut_pmuxcr_val = 0;

        gut_pmuxcr_val = *(u32*) CONFIG_GUT_PMUXCR;
        gut_pmuxcr_val |= (1<<18);
        *(u32*) CONFIG_GUT_PMUXCR = gut_pmuxcr_val;
    }
#endif
	/* Reset the MAC */
...
//#ifdef CONFIG_BOREN_ETHNET_SETUP
#if 0
    printf("Force Link Up...\n");
	/* force link up */
	priv->link = 1;
#else /* end of CONFIG_BOREN_ETHNET_SETUP */
	/*
	...
	printf("link here %d\n", priv->link);
#endif
#endif /* CONFIG_BOREN_ETHNET_SETUP */
	return 0;
}
...
#if 0
//#ifdef CONFIG_BOREN_ETHNET_SETUP
	printf("Force Link Speed & Duplex...\n");
	priv->speed = 1000;
	priv->duplexity = 1;
#else	
...
static void startup_tsec(struct eth_device *dev)
{
    ...
    //printf("%d,%d,%d\n",priv->link,priv->speed,priv->duplexity);
	/* Start up the PHY */
	if(priv->phyinfo)
 ...
 * errors
 */
static void sc_dump_packet(unsigned char *buf, int len)
{
	int i = 0;
...
static int tsec_recv(struct eth_device *dev)
{
	...
	while (!(rtx.rxbd[rxIdx].status & RXBD_EMPTY)) {
		...
		//sc_dump_packet(NetRxPackets[rxIdx], length-4);
...
#ifdef CONFIG_WGXTM_BOREN /* add PHYID support for Aruba_k2's marvell88e1512 GE PHY */
...
#endif
#ifdef CONFIG_WGXTM_BOREN /* add PHYID support for AP331IB's BCM54616S GE PHY */
static struct phy_info phy_info_BCM54616S = {
	0x3625d12,	/* 5461 ID */
	"Broadcom BCM54616S",
...
static struct phy_info *phy_info[] = {
	&phy_info_cis8204,
	&phy_info_cis8201,
#ifdef CONFIG_WGXTM_BOREN /* TONY_FOR_AP331IB */
	&phy_info_BCM54616S,
	&phy_info_M88E1512,
#endif
	&phy_info_BCM5461S,
