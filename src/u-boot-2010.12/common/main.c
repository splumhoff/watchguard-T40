...
#define BUTTON_RESET			(0x01)	/*GPIO PIN 5 from GPIO data register (GPIO_GPDAT): P1020RM.pdf: 20.3.3
						 * NOT sure what the datasheet of Boren says but this one works
						 */

#define BOREN_FACTORY_RESET_GPIO	0x40000000
#define SOMETHING_FACTORY_RESET_GPIO	0x00200000
/*
...
int read_btn_status(unsigned int btn)
{
	...
	if (reg_val & (1 << (31 - btn)) )
		...
		return 1;
#elif CONFIG_SOMETHING
	/* Low active */
	if (reg_val & SOMETHING_FACTORY_RESET_GPIO)
		return 0;
	else
		return 1;
#else
...
#endif // ifdef CONFIG_WGXTM_BOREN

#ifdef CONFIG_BOREN_ETHNET_SETUP

void sc_set_register(int address,int bit)
{
    REG32(address)  |=  (1<<(31-bit));

}
int sc_get_flag(void)
{
    char buf[1];
    memcpy(buf, (void *)(0xee140000), 1);
    return (buf[0]);
}
#endif
void main_loop (void)
