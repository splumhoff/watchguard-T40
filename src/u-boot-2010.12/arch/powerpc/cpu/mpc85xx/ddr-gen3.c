...
/*TONY*/
#if 1
#define DSO_MDIC_EN     30
#define DHC_EN          31
#define DSO_MDICPZ      26
#define DSO_MDICNZ      22
#define DSO_MDIC_PZ_OE  21
#define DSO_MDIC_NZ_OE  20
...
#error
	...
	u32 sdr1_val;
	int j=0;
	unsigned char cal_data[]={0x1, 0x3, 0x2, 0x6, 0x7, 0x5, 0x4, 0xC, 0xD, 0xE, 0xA, 0xB, 0x9, 0x8, 0x0};

	printf("dsr1 %x\n", in_be32(&ddr->ddr_dsr1));
	cdr1_val = in_be32(&ddr->ddr_cdr1);
	//cdr1_val |= (1<<DSO_D_EN);
	//out_be32(&ddr->ddr_cdr1, cdr1_val);
	cdr1_val &= ~(1<<DHC_EN); /* set to use manual calibration */
	//out_be32(&ddr->ddr_cdr1, cdr1_val);
	cdr1_val |= (1<<DSO_MDIC_EN);
	printf("set cdr to %x\n", cdr1_val);
	out_be32(&ddr->ddr_cdr1, cdr1_val);
	...
	for(j=0; j<15; j++) {
		...
		cdr1_val = in_be32(&ddr->ddr_cdr1);
		cdr1_val |= (cal_data[14-j] << DSO_MDICPZ);
		printf("setting MDICPZ as %x\n", cal_data[14-j]);
		out_be32(&ddr->ddr_cdr1, cdr1_val);


		cdr1_val = in_be32(&ddr->ddr_cdr1);
		cdr1_val |= (1 << DSO_MDIC_PZ_OE);
		out_be32(&ddr->ddr_cdr1, cdr1_val);
		...
		sdr1_val = in_be32(&ddr->ddr_dsr1);
		if(sdr1_val & (1<<31))
			break;
	}

	printf("dsr1 %x\n", sdr1_val);
	printf("cdr1 %x\n", in_be32(&ddr->ddr_cdr1));
	cdr1_val = in_be32(&ddr->ddr_cdr1);
	cdr1_val &= ~(1<<DSO_MDIC_PZ_OE);
	printf("set cdr to %x\n", cdr1_val);
	out_be32(&ddr->ddr_cdr1, cdr1_val);
	...
	for(j=0; j<15; j++) {
		...
		cdr1_val = in_be32(&ddr->ddr_cdr1);
		cdr1_val |= (cal_data[14-j] << DSO_MDICNZ);
		out_be32(&ddr->ddr_cdr1, cdr1_val);

		cdr1_val = in_be32(&ddr->ddr_cdr1);
		cdr1_val |= (1 << DSO_MDIC_NZ_OE);
		printf("setting MDICNZ as %x\n", cal_data[14-j]);
		out_be32(&ddr->ddr_cdr1, cdr1_val);
	...
	printf("cdr1 %x\n", in_be32(&ddr->ddr_cdr1));
	printf("dsr1 %x\n", in_be32(&ddr->ddr_dsr1));
	//out_be32(&ddr->ddr_cdr1, 0x7b838888);
	out_be32(&ddr->ddr_cdr1, 0x62038888);
	printf("cdr1 %x\n", in_be32(&ddr->ddr_cdr1));
