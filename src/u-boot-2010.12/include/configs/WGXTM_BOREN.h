...
#define	CONFIG_EXTRA_ENV_SETTINGS				\
	...
	MK_STR(CONFIG_BOREN_BOARD_SPECIFIC_ENV_FDT) "\0"	\
	MK_STR(CONFIG_BOREN_BOARD_SPECIFIC_ENV_WGKERNEL) "\0"	\
	...
	"bootm $loadaddr - $fdtaddr;\0"             \
	"wgBootRecovery=setenv bootargs wgmode=safe root=/dev/$SysARoot rw rootdelay=2 " \
	"console=$consoledev,$baudrate $othbootargs; "	\
	"mmcinfo; " \
	"ext2load mmc 0:3 $fdtaddr $fdtfile; " \
	"ext2load mmc 0:3 $loadaddr $WGKernelfile; " \
