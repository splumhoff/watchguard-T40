	...
	int flags)
{
	...
	extern int incr_ethaddr(uchar *enetaddr, uchar incVal);
    if(!strncmp(name, "ethaddr", strlen(name)))
    {
...
#endif
		{
            ...
            else /* env_op_create and env_op_overwrite */
            {
				eth_parse_enetaddr(value, tmpEnetaddr);
                incr_ethaddr(tmpEnetaddr, idx+7); //undefined reference
            }
