...
//Add by Wu Steven. B
#define AQUANTIA_GLOBAL_LED_PROVISION0		0xc430
#define AQUANTIA_GLOBAL_LED_PROVISION1		0xc431
#define AQUANTIA_GLOBAL_LED_PROVISION2		0xc432
...
//Add by Wu Steven. B
static int aquantia_led_config(struct phy_device *phydev)
{
	u16 reg;
	reg = phy_read(phydev, MDIO_MMD_VEND1, AQUANTIA_GLOBAL_LED_PROVISION0);
	debug("%s LED0 0x%04x Before\n", phydev->dev->name, reg);
	phy_write(phydev, MDIO_MMD_VEND1, AQUANTIA_GLOBAL_LED_PROVISION0,
			  AQUANTIA_GLOBAL_LED0);
	reg = phy_read(phydev, MDIO_MMD_VEND1, AQUANTIA_GLOBAL_LED_PROVISION0);
	debug("%s LED0 0x%04x After\n", phydev->dev->name, reg);
	reg = phy_read(phydev, MDIO_MMD_VEND1, AQUANTIA_GLOBAL_LED_PROVISION1);
	debug("%s LED1 0x%04x Before\n", phydev->dev->name, reg);
	phy_write(phydev, MDIO_MMD_VEND1, AQUANTIA_GLOBAL_LED_PROVISION1,
			  AQUANTIA_GLOBAL_LED1);
	reg = phy_read(phydev, MDIO_MMD_VEND1, AQUANTIA_GLOBAL_LED_PROVISION1);
	debug("%s LED1 0x%04x After\n", phydev->dev->name, reg);
	reg = phy_read(phydev, MDIO_MMD_VEND1, AQUANTIA_GLOBAL_LED_PROVISION2);
	debug("%s LED2 0x%04x Before\n", phydev->dev->name, reg);
	phy_write(phydev, MDIO_MMD_VEND1, AQUANTIA_GLOBAL_LED_PROVISION2,
			  AQUANTIA_GLOBAL_LED2);
	reg = phy_read(phydev, MDIO_MMD_VEND1, AQUANTIA_GLOBAL_LED_PROVISION2);
