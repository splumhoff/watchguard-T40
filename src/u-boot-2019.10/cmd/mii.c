          ...
          && (!strncmp(argv[argc - 1], "mvl", strlen("mvl"))))
    {
		...
		for (addr = addrlo; addr <= addrhi; addr++) {
			for (reg = reglo; reg <= reghi; reg++) {
				...
				else if (!strncmp(argv[4], "srds", strlen("srds")))
				{
		...
		for (addr = addrlo; addr <= addrhi; addr++) {
			for (reg = reglo; reg <= reghi; reg++) {
                ...
                else if (!strncmp(argv[5], "srds", strlen("srds")))
				{
					unsigned short offset= 0x8100 + reg;
...
U_BOOT_CMD(
	...
	"mii read     <addr> <reg>        srds  - Marvell Serdes read  MII PHY <addr> register <reg>\n"
    "mii write    <addr> <reg> <data> srds  - Marvell Serdes write MII PHY <addr> register <reg>\n"
#endif
